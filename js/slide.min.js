...
const handleChangeSlide = () => {
    if(current == length -1){
        current = 0
         ...
         listImage.style.transform= `translate(0px)`
         document.querySelector('.active').classList.remove('active')
         document.querySelector('.index-item-'+ current).classList.add('active')
    }else {
        current ++
        let width = imgs[0].offsetWidth
        listImage.style.transform= `translate(${width * -1 * current}px)`
        document.querySelector('.active').classList.remove('active')
         document.querySelector('.index-item-'+ current).classList.add('active')
    }
}
...
const bulletActiveSpans = document.querySelector('.swiper-pagination');

  console.log(bulletActiveSpans,'swiper_pagination');